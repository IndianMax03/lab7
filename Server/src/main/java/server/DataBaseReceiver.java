...
public class DataBaseReceiver {
    ...
    private DataBaseReceiver() {
        try {
            connection = DriverManager.getConnection("jdbc:postgresql://localhost:5432/laba7", "labber", "laba");
            PreparedStatement statement = connection
                    .prepareStatement("create table if not exists users(" + "login varchar(20) not null primary key,"
                            + "password varchar(150)" + ");" + "create table if not exists cities("
                            + "id serial not null primary key," + "name varchar(50) not null unique,"
                            + "x double precision not null," + "y double precision not null," + "creation_date date,"
                            + "area float," + "population integer," + "meters_above_sea_level float,"
                            + "climate varchar(23)," + "government varchar(13)," + "standard_of_living varchar(13),"
                            + "governor varchar(18)," + "governor_height integer," + "governor_birthday date,"
                            + "login varchar(20)," + "foreign key (login) references users (login)" + ");");
    ...
    public boolean removeAllByGovernment(String government, String login) {
        try {
            PreparedStatement statement = connection
                    .prepareStatement("delete from cities where government = ? and login = ?;");
    ...
    public boolean removeGreater(City city, String login) {
        try {
            PreparedStatement statement = connection
                    .prepareStatement("delete from cities where name > ? and login = ?;");
            statement.setString(1, city.getName());
            statement.setString(2, login);
            statement.executeUpdate();
        } catch (SQLException throwables) {
    ...
    public boolean update(int id, City city, String login) {
        try {
            PreparedStatement statement = connection
                    .prepareStatement("select count(*) from cities where id = ? and login = ?;");
            statement.setInt(1, id);
            statement.setString(2, login);
            ResultSet result = statement.executeQuery();
            result.next();
            ...
            while (result.next()) {
                ...
                if (city != null && !collection.add(city)) {
                    logger.log(Level.WARNING,
                            "ВНИМАНИЕ. В БАЗЕ ДАННЫХ ОБНАРУЖЕН ГОРОД, НАРУШАЮЩИЙ УСЛОВИЕ ПОЛЬЗОВАНИЯ КОЛЛЕКЦИЕЙ. "
